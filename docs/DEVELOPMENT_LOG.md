...
- 2026-07-23: Correção do cálculo de fatura em `useFinancialSummary.getMonthlySummary()`: `cardStatements` agora inclui lançamentos de ENTRADA (`type='income'`) com `card_id`, subtraindo-os do total de despesas (e não apenas somando despesas). Isso permite registrar créditos/devoluções direto no cartão e reduzir corretamente a dívida da fatura sem criar um registro separado de reembolso vinculado.
- 2026-07-23: Regra de competência do cartão ajustada para usar o dia de fechamento como divisor da fatura em `getCardFinancialEffectiveDate()`, garantindo que lançamentos antes do fechamento permaneçam no mês atual e lançamentos após o fechamento vão para o mês seguinte; validação determinística reforçada com cenário de 20/06 e 30/06.
- 2026-07-23: Aba `Contas` em `/transactions` passou a aplicar competência financeira de fatura para reembolsos vinculados ao cartão (mês de filtro, faixa de dia, exibição de competência e exportação), mantendo `instance_date` apenas para lançamentos não vinculados.

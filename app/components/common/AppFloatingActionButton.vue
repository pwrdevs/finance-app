...
async function handleClick() {
  ...
  await router.push({ path: '/transactions', query: { new: '1' } })
}
</script>
...
<template>
  <button
    v-if="shouldShowFab"
    type="button"
    class="fixed bottom-[calc(1rem+env(safe-area-inset-bottom))] right-4 z-40 inline-flex h-12 w-12 items-center justify-center rounded-full bg-primary-dark text-surface shadow-panel transition hover:-translate-y-0.5 hover:bg-primary focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary-dark/60 sm:bottom-6 sm:right-6 sm:h-14 sm:w-14"
